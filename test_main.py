...
def get_session_id(name: str) -> str:
    ...
    # else:
    #     print("Сессия получена -", session_id)
    return session_id
...
if __name__ == '__main__':
    while True:
        time_now = datetime.datetime.now() + datetime.timedelta(hours=3)  # смещение на американском сервере + 3ч
        h = time_now.hour
        m = time_now.minute
